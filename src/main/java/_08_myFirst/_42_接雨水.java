...
public class _42_接雨水 {  //  	todo 困难
    ...
    public int trap0(int[] height) {
        ...
        int[] leftMaxes = new int[height.length];    // 最左 最右 装不了水。故，i的取值范围是 [ 1,height.length-2 ]
        for (int i = 1; i <= lastIdx; i++) {        //  i 位置左边最大值 = max(前一次算的最大值，和前一个位置的高度)
            leftMaxes[i] = Math.max(leftMaxes[i - 1], height[i - 1]);
        }

        int[] rightMaxes = new int[height.length];  // i 位置右边最大值
    ...
    public int trap(int[] height) {
        ...
        for (int i = 1; i <= lastIdx; i++) {
            ...
            // 求出左边最大、右边最大中的较小者
            int min = Math.min(leftMax, 0); //  TODO bug
            // 说明这根柱子不能放水
