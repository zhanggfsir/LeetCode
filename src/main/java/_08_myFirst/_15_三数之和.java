...
public class _15_三数之和 {
    ...
    public List<List<Integer>> threeSum(int arr[]){
        ...
        for(int i=0;i<arr.length-2;i++){ // 为什么要 arr.length-2:arr.length-1 为倒数第1个数;arr.length-2 为倒数第2个数
            // 当前元素不能和前一个元素相等。放行条件：1 i=0;2 (i>0 && arr[i]!=arr[i-1])
            if(i==0 || (i>0 && arr[i]!=arr[i-1])){
                int low=i+1,high=arr.length-1,sum=0-arr[i]; // 下标low high，两数之和sum
                while (low<high){
