...
public class _27_RemoveElement {
    ...
    public static int removeElement(int[] nums, int val) {
        int i = 0;
        for (int j = 0; j < nums.length; j++) {  // 可能第一个值就是 val，故j从0开始
            if (nums[j] != val) {  // 当不满足条件时 i++  去重是和前一个元素arr[i]比较；剔除制定元素val 是和制定元素val比较
                nums[i] = nums[j];
                i++;                // 由于第一个值可能就是 val，故需要之后进行 i++
            }
        }
        return i;
    }
