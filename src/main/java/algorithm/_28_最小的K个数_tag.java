...
public class _28_最小的K个数_tag {
    ...
    最小的k个元素--建大顶堆 ; 找最大的k的元素--建小顶堆 ！！！
    大顶堆：每个结点的值都大于或等于其左右孩子结点的 ！！
    适合海量数据的最大堆 O（nlogk）  k个数的堆，空间固定,k个数字的最大堆 !!
    ...
    */
    public class Solution2 {
        public ArrayList<Integer> GetLeastNumbers_Solution_2(int [] input, int k) {
            ...
            // 构造优先队列，排序方法是自然数顺序的逆序，所以是个最大堆，这样这个堆的堆顶就是k个数中的最大数
            Queue<Integer> queue = new PriorityQueue<>(k, Collections.reverseOrder());
